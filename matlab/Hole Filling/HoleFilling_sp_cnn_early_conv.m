function HoleFilling_sp_cnn_early_conv()
...
%tests = {'filledSquare'};
raw = cell(length(tests) + 1, 5 + 1 + prod(512 ./ cnn_dim) + 1 + max(numUnits));
...
my_csvwrite('early_finish_conv_results/results.csv', raw);
end
...
[runData, state, output ] = sp_cnn_conv('early-finish', cnn_dim, r, A, B, z, initState, u, ...
    partitions, interval, numCNN );
file_header = sprintf('early_finish_conv_results/%s_%d', test_name, numCNN);
