...
//    }
    ...
    size_t pos1 = optionFormatName_.find("[ ");
    optionDisplayName_ = option->format_name();
    if (pos1 != std::string::npos) {
    	hasShort_ = true;
    	size_t pos2 = optionFormatName_.find(" ]");
    	optionID_ = optionFormatName_.substr(pos1 + 4, pos2 - pos1 - 4);
    } else {
    	hasShort_ = false;
    	optionID_ = optionFormatName_.substr(2);
    }
  }
...
//---------------------------------------------------------------------------------------------------------------------
  void CustomOptionDescription::checkIfPositional(const boost::program_options::positional_options_description& positionalDesc)
  {
    ...
    {
    	std::cout << optionID_ << " - ";
    	std::cout << positionalDesc.name_for_position(i) << std::endl;
